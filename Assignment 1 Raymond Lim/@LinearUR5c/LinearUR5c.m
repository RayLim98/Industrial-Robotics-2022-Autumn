...
function self = LinearUR5c(useGripper)
...
%     end
    ...
    L(2) = Link('d',0.1599,'a',0,'alpha',-pi/2,'offset',pi/2);
...
%     L(5).offset = -pi/2;
    ...
    self.model.base = self.model.base * transl(0,0.5,0) * trotx(pi/2) * troty(pi/2);
end
%% PlotAndColourRobot
...
function PlotAndColourRobot(self)%robot,workspace)
    ...
    for linkIndex = 0:self.model.n
        ...
        display(['Vertexx size: ', num2str(size(vertexData))])
    end
    % Display robot
    ...
    display(['Finished coloring ', num2str(toc)]);
end        
    end
end