...
methods (Static)
    ...
    function RunPart1()
        ...
        pause
        axis equal; % This will make the axis square
        pause
        %% Transform 2
        T2 = se2(2, 1, 0)
        hold on
        ...
        for i=1:2:90    
            try delete(R_h); end % Need to put the delete inside try so it won't crash if R_h dosen't exist
            R = rotx(i * pi/180)
            R_h = trplot(R,'frame','1', 'rgb','arrow');
            drawnow();
        %     pause(0.01);
    ...
    function RunPart3()
        ...
        display('The qlim in radians is [minQ1, maxQ1; minQ2, maxQ2]');
        qlim = robot.qlim
        robot.plot(qlim(1,:))
        robot.teach;
...
methods
    function self = Lab2Starter()
        display('This is the constructor and does not really need to be here if it does nothing like this')
    end
    ...
    function ChangeValues(self)
        ...
        try delete(self.classProperty3); end
        self.classProperty3 = [];       
    end
end
